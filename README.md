...
Cada feature segue este ciclo obrigatório — **nada entra pela metade**:

```
main (sempre compilável)
 └─ dev (integração)
     └─ feat/<nome> (uma branch por feature)
```

1. Planeamos a feature (humano + Claude Sonnet 4.6)
2. Agente implementa em `feat/<nome>`
3. Testes escritos e a passar
4. PR aberto pelo agente via `gh pr create`
5. Review por Gemini 3.1 Pro · `thinking_level: medium`
6. Aprovação humana
7. Merge para `dev` por agente (Haiku 4.5)
8. Logbook actualizado neste README

### Routing de modelos de IA
...
| PR review | Gemini 3.1 Pro | `thinking_level: medium` |
| Debug difícil | Sonnet 4.6 | `effort: medium` |
| Decisão arquitectural | Sonnet 4.6 | `effort: high` |
| Último recurso | Opus 4.8 | `effort: high` |

> **Nota:** Haiku 4.5 não suporta o parâmetro `effort` — usa `budget_tokens`
> manual. O parâmetro `effort` (low / medium / high / xhigh / max) pertence
> ao Sonnet 4.6 e Opus 4.7/4.8.
