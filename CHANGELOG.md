# mise — Registo de trabalho

O que já foi feito, com as decisões e as dificuldades reais de cada etapa.
Saiu do `PROJECT.md` a 2026-07-28: o plano tinha 1800 linhas e 80 % eram
histórico, o que tornava impossível ver o que faltava. **O plano é o
`PROJECT.md`; isto é a memória.**

Não é um changelog de versões — é o registo do porquê. Consulta-se quando é
preciso saber *porque* é que uma coisa ficou como ficou, antes de a mudar.

**Convenção:** cada etapa fechada acrescenta aqui a sua secção, incluindo o
que correu mal. As dificuldades ficam registadas de propósito — foram elas
que produziram as regras que hoje parecem arbitrárias (transações nas
escritas multi-tabela, unidades normalizadas na escrita, `nix-shell` para
todo o `cargo`, uma migração por transformação de dados).

Documentos arquivados relacionados, em `legacy/`:

- `legacy/AUDIT-2026-07-20-externa.md` — auditoria externa de 2026-07-20.
  Substituída pela interna de 2026-07-26, que reverificou todas as suas
  afirmações contra o código e encontrou erros em várias. O estado por achado
  está aqui, na secção «Auditoria externa (2026-07-20)».
- `legacy/HANDOFF_AGENT.md` — log técnico de sessões até 2026-07-05.
...
- [ ] **Bug: câmara não abre no Scanner de recibos** (sessão de 2026-07-05,
  ...
  **PRIORIDADE ALTA (revisão 2026-07-10):** deixa de ser um item que se
  ...
  ser portada para Android (`PROJECT.md` §Alvos de distribuição), onde a câmara é a
  ...
  *enumerar* o dispositivo mesmo ele estando acessível e funcional fora
  ...
  desktop **não bloqueia o arranque dos `PROJECT.md` §Alvos de distribuição** — as duas
...
- [x] **Empacotamento Linux** ✅ CONCLUÍDA (2026-07-10) — `cargo tauri build` com bundles `.deb` e
  ...
  **Metadados de bundle preenchidos (2026-07-10):** `src-tauri/tauri.conf.json`
  ...
  conjuntos Android/iOS já prontos para os `PROJECT.md` §Alvos de distribuição). Adicionado
...
Relatório completo em `legacy/AUDIT-2026-07-20-externa.md` (modelo externo, método estático —
...
| DOM-009 | Sem schema para waste/histórico de stock | ⏳ Aberto, e **subestimado aqui**: não é "feature nova se houver pedido", é metade do domínio em falta. Passou a `PROJECT.md` §Desenho — Movimentos de stock, o próximo trabalho estruturante. |
...
| FUN-004 | Rota `/sugestor` morta (stub) | ✅ Corrigido 2026-07-20 (`2e3868a`) — rota e `PlaceholderPage.tsx` removidos. O backend `suggester_suggest` foi apagado a 2026-07-27 (`8bbc975`): era um stub sem base de decisão possível enquanto não houver consumo registado. Volta com `PROJECT.md` §Desenho — Movimentos de stock. |
...
| Cluster P3 | Notas de polish e positivas (ARC-003/005/006/007, DOM-006/007/010, FUN-005/006/007, UX-001/005/006/007, SEC-005/006/008/009, QA-003, BLD-007) | Sem ação — ver `legacy/AUDIT-2026-07-20-externa.md` para detalhe, nada aqui muda o roadmap. |
...
2026-07-20, esta não aceitou como verdade nem o `legacy/AUDIT-2026-07-20-externa.md` nem as
...
| ID | Decisão |
|----|---------|
| PRD-01 | Não é um fix — é `PROJECT.md` §Desenho — Movimentos de stock, o próximo trabalho estruturante |
...
| `create_shopping_list_from_recipes` | §2.1 da auditoria classificou-o bem («Prometido»); a proposta de o apagar foi minha, e estava errada. É feature por implementar, ver `PROJECT.md` §Desenho — Lista de compras a partir de receitas |
