  ...
  const columns = [
    {
      ...
      render: (item: any) => (
        <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)" }}>
          ...
          <span className="text-4 mono text-muted">
            ({UNIT_LABELS[getIngredientUnit(item.ingredient_id)] ?? getIngredientUnit(item.ingredient_id)})
          </span>
        </div>
      )
    },
    { key: "quantity", label: "Qtd Actual", align: "right" as const, render: (item: any) => <span className="mono">{item.quantity}</span> },
    { key: "min_quantity", label: "Mínimo", align: "right" as const, render: (item: any) => <span className="mono">{item.min_quantity}</span> },
    { 
      key: "status", 
      label: "Estado", 
      render: (item: any) => {
    ...
    },
    {
      ...
      render: (item: any) => (
        <div className="ingredient-actions" role="group">
          <button className="btn-icon" onClick={(e) => { e.stopPropagation(); onEdit(item); }} title="Editar">
      ...
      <div style={{ maxHeight: "65vh", overflowY: "auto", paddingRight: "var(--space-2)" }}>
        ...
        <div style={{ marginTop: "var(--space-4)", padding: "var(--space-3)", background: "var(--color-surface)", borderRadius: "var(--radius-md)", border: "1px solid var(--color-border)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span className="text-2">Total: <strong className="currency">{(form.quantity * form.price_per_unit).toFixed(2)} €</strong></span>
            <span className="text-2 mono text-muted">Subtotal: {(form.quantity * form.price_per_unit).toFixed(2)} €</span>
          </div>
        ...
        {purchases.length > 0 && (
          <div style={{ marginTop: "var(--space-6)" }}>
            ...
            <DataTable
              columns={[
                { key: "purchase_date", label: "Data", render: (p: any) => <span className="mono">{p.purchase_date.split("T")[0]}</span> },
                { key: "quantity", label: "Qtd", render: (p: any) => <span className="mono">{p.quantity} {UNIT_LABELS[p.unit] ?? p.unit}</span> },
                { key: "price_per_unit", label: "Preço/unid", render: (p: any) => <span className="mono">{p.price_per_unit.toFixed(2)} €</span> },
                { key: "total_price", label: "Total", render: (p: any) => <span className="mono">{p.total_price.toFixed(2)} €</span> },
                { key: "discount", label: "Promoção", render: (p: any) => p.is_discount ? <span className="badge badge-success">{p.discount_percent}%</span> : <span className="text-muted">—</span> },
                { key: "supplier_name", label: "Fornecedor", render: (p: any) => p.supplier_name ?? "—" }
              ]}
              rows={purchases}
            />
          </div>
